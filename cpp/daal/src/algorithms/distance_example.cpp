...
#include "../../include/algorithms/distances.h"
...
{
    std::cout << "Distance example:" << std::endl << std::endl;

    const size_t nObservations = 500;
    const size_t nFeatures     = 10;
    const size_t nRead         = 10;
    const size_t firstReadRow  = 0;

    string name1 = "";
    string tmp = "";
    ifstream input1("../../../../examples/daal/data/batch/distance1.txt");
    ...
    float data1[nFeatures * nObservations] = {};
    for (size_t i = 0, j = 0; i < name1.size(); ++i)
    {
        if (name1[i] != ',')
        {
            tmp += name1[i];
    ...
    HomogenNumericTable<> homogenDataTable1(data1, nFeatures, nObservations);

    NumericTablePtr dataTable1(&homogenDataTable1);

    checkPtr(dataTable1.get());
    BlockDescriptor<> block1;

    dataTable1->getBlockOfRows(firstReadRow, nRead, readOnly, block1);
    std::cout << block1.getNumberOfRows() << std::endl;
    printArray<float>(block1.getBlockPtr(), nRead, block1.getNumberOfRows(), "Print FIRST:");

    string name2 = "";
    ifstream input2("../../../../examples/daal/data/batch/distance2.txt");
    for (size_t i = 0; i < 500; ++i) {
        input2 >> tmp;
        name2 += tmp;
    }
    ...
    tmp = "";
    float data2[nFeatures * nObservations] = {};
    for (size_t i = 0, j = 0; i < name2.size(); ++i)
    {
        if (name2[i] != ',')
        {
            tmp += name2[i];
    ...
    HomogenNumericTable<> homogenDataTable2(data2, nFeatures, nObservations);

    NumericTablePtr dataTable2(&homogenDataTable2);
    checkPtr(dataTable2.get());
    BlockDescriptor<> block2;

    dataTable2->getBlockOfRows(firstReadRow, nRead, readOnly, block2);
    std::cout << block2.getNumberOfRows() << std::endl;
    printArray<float>(block2.getBlockPtr(), nRead, block2.getNumberOfRows(), "Print SECOND:");
    

    float data3[nObservations * nObservations] = {};

    HomogenNumericTable<> homogenDataTable3(data3, nObservations, nObservations);

    NumericTablePtr distancesTable(&homogenDataTable3);
    checkPtr(distancesTable.get());

    distances::minkowski<float>(dataTable1, dataTable2, distancesTable, size_t(3));

    BlockDescriptor<> block;
    
    distancesTable->getBlockOfRows(firstReadRow, nRead, readOnly, block);
    std::cout << block.getNumberOfRows() << std::endl;
    printArray<float>(block.getBlockPtr(), nRead, block.getNumberOfRows(), "Print RESULT:");
    distancesTable->releaseBlockOfRows(block);
    return 0;
}